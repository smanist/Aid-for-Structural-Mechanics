...
varD1D[EXPR_,FUN_,VAR_]:=Module[{expr=EXPR,fun=FUN,var=VAR,t=ToExpression["t"],res,df,tmp,tmpVar,prt,prt0},
(*Variantional operation on Integrals based on Gateaux derivative*)
(*Specialized for Hamilton's Principle*)
res=Table[
...
(*Modify terms with temporal variable*)
prt={
...
prt0=tmp-prt[[1]]D[df[var,t],t]-prt[[2]]D[df[var,t],t,x]-prt[[3]]D[df[var,t],t,x,x];
(*terms not related to time + terms from dt + terms from dtx + terms from dtxx*)
tmp=prt0-D[prt[[1]],t]df[var,t]-D[prt[[2]],t]D[df[var,t],x]-D[prt[[3]],t]D[df[var,t],x,x];
(*Modify terms with spatial variable*)
prt={
...
Module[{expr=EXPR,fun=FUN,var=VAR,tuw,\[CapitalLambda]=ToExpression[{"\[Lambda]l","\[Lambda]r"}],t=ToExpression["t"],eb,eqd,eqb,bcl,bcr,res},
(*Derive Hamiltonian EOM of a beam with BC's on both ends*)
(*Convert energy expression to normal form, and Seperate domain terms and boundary terms*)
tuw=Normal[CoefficientArrays[#,\[CapitalLambda]]]&[energyParser[-expr[[1]]+expr[[2]]-expr[[3]],fun,var,Table[f[var,t],{f,fun}]]];
tuw=If[Length[tuw]==1,{tuw[[1]],{0,0}},tuw];
(*Do variation on energy, domain terms*)
eqd=varD1D[tuw[[1]],fun,var];
(*Do variation on energy, boundary terms*)
eqb=varD1D[tuw[[2]],fun,var];
(*Assemble the EOM*)
res=Table[
(*The variable*)
{eqd[[idx,1]],
...
eqd[[idx,2]],
(*The left BC*)
{{fun[[idx]][var,t],-eqd[[idx,3]]+eqb[[idx,2,1]]},
{D[fun[[idx]][var,t],var],-eqd[[idx,4]]+eqb[[idx,3,1]]}},
(*The right BC*)
{{fun[[idx]][var,t],eqd[[idx,3]]+eqb[[idx,2,2]]},
{D[fun[[idx]][var,t],var],eqd[[idx,4]]+eqb[[idx,3,2]]}}},
{idx,1,Length[fun]}];
res
];
...
qfun,cnt,tar,lentmp,tuw,ener,dq,KK,MM,res,f0,fq,fdq},
...
tuw=CoefficientArrays[energyParser[expr[[1]]-expr[[2]]+expr[[3]],fun,var,tar],\[CapitalLambda]]//Normal;
tuw=If[Length[tuw]==1,{tuw[[1]],{0,0}},tuw];
(*Derive the total energy, including boundary terms*)
ener=Expand[Integrate[tuw[[1]],Flatten@{var,rng}]+(tuw[[2,1]]/.{var->rng[[1]]})+(tuw[[2,2]]/.{var->rng[[2]]})];
